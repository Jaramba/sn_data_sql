SELECT 
  EXTRACT (DOW FROM orders.create_date) ,
  COUNT(*)
FROM crm_lead AS orders 
WHERE orders.create_date >= '2017-01-01' AND
 orders.create_date <= '2018-08-31'
...
SELECT * FROM crm_lead LIMIT 10
SELECT * FROM crm_lead2opportunity_partner
...
invoices_upgrades AS (
	SELECT  
 		...
 		products."name" 												AS "Product",
   		...
   		COALESCE(trim(leads.x_referred),cc."StaffName")						AS "Referred By",		
	...
	LEFT JOIN res_users AS sales_people ON  
	 ...
	 WHERE  
 		...
 		client_type."name" = '2. Returning Client' 
 		--AND 
...
invoices_new AS (
	SELECT 
 		...
 		products."name" 												AS "Product",
   		branches."name"													AS "Branch Name",
   		COALESCE(cc."StaffName",cc2."StaffName")						AS "Comment/Lead Creator",
...
FROM combined_data
WHERE 
	combined_data."Date" >= '2018-01-01' AND
	combined_data."Date" <= '2018-08-31' AND
	combined_data."Country" = 1 
